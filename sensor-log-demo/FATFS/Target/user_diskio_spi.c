...
#include "stm32wlxx_hal.h"		// HAL functions for STM32WL
#include "user_diskio_spi.h"	// FatFs SPI interface prototypes
#include "main.h"				// Contains SD_SPI_HANDLE and CS pin definitions
extern void myprintf(const char *fmt, ...); // UART debug print put in during debugging, not needed anymore
...
// Chip select macros for SD card SPI
