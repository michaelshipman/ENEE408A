...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	/*
	 ...
	 status = sh2_open(&bno_sh, eventCallback, NULL);
	 status = sh2_getProdIds(&bno_id);
	 status = sh2_setSensorCallback(sensorCallback, NULL);

	 bno_config.changeSensitivityEnabled = false;
	 bno_config.wakeupEnabled = false;
	 bno_config.changeSensitivityRelative = false;
	 bno_config.alwaysOnEnabled = false;
	 bno_config.changeSensitivity = 0;
	 bno_config.batchInterval_us = 0;
	 bno_config.sensorSpecific = 0;
	 bno_config.reportInterval_us = 10000;

	 status = sh2_setSensorConfig(reportType, &bno_config);

	 bno_config.reportInterval_us = 20000;

	 status = sh2_getProdIds(&bno_id2);

	 status = sh2_getSensorConfig(reportType, &bno_config2);
	 */
